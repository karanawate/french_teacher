						...
						<div class="card border-top border-0 border-4 border-danger">
							<div class="card-body p-5">
								<div class="card-title d-flex align-items-center">
									...
									<h5 class="mb-0 text-danger">Upload Teacher Review</h5>
								</div>
								<hr>
								<div class="col-md-12">
									
								<form class="row g-3" action="addteacher" method="post" enctype= multipart/form-data >
								@csrf
									<div class="col-md-6">
										...
										<div class="input-group"> 
											...
											<div class="col-md-6">
												<label for="inputLastName1" class="form-label">Upload Teacher Image</label>
												<div class="input-group"> 
													<input type="file" name="teacherImage" class="form-control border-start-0" id="inputLastName1" placeholder="" />
												</div>
												<span style="color:red">@error('teacherImage'){{ $message }}@enderror</span>
											</div>
											
											  <div class="col-md-6">
												<label for="inputLastName1" class="form-label">Enter Teacher Head</label>
												 <div class="input-group"> 
													<input type="text" name="teacherHead" value="" class="form-control" id="" placeholder="Enter Teacher Head" />
												 </div>
												 <span style="color:red">@error('teacherHead'){{ $message }}@enderror</span>
										  	</div>
							...
							<table id="example2" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
										...
										<td><img src="{{url('teacherImage/'.$value->teacherImage)}}" width="100px;" height="100px;"></td>
										<td>@php echo substr($value->testDesc,0,100) @endphp</td>
										<td>
											<a href="{{ url('Testmonialedit/'.$value->teachId) }}"><i class="fadeIn animated bx bx-message-square-edit" style="font-size:30px;"></i></a>
											<form class="" method="post" onclick="return check()" action="{{ url('deleteteacherId')}}" >
											@csrf
												<input type="hidden" name="teachId" value="{{$value->teachId }} " >
...
@if(isset($colors))
	<script type="text/javascript">
		setTimeout(function() {
			window.location = "{{ url('add-teacher') }} ";  
	}, 2000);
